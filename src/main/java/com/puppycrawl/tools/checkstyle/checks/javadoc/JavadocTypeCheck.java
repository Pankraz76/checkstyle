...
public class JavadocTypeCheck
        ...
        if (shouldCheck(ast)) {
            ...
            if (textBlock != null) {
                ...
                if (!allowMissingParamTags) {
                    ...
                    typeParamNames.forEach(typeParamName -> checkTypeParamTag(ast, tags, typeParamName));

                    recordComponentNames.forEach(componentName -> checkComponentParamTag(ast, tags, componentName));
                }
    ...
    private List<JavadocTag> getJavadocTags(TextBlock textBlock) {
        ...
        if (!allowUnknownTags) {
            tags.getInvalidTags().forEach(tag -> log(tag.getLine(), tag.getCol(), MSG_UNKNOWN_TAG, tag.getName()));
        }
